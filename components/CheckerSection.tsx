...
interface Finding { icon: string; title: string; detail: string }
interface ForensicFlag { icon: string; label: string; value: string; status: 'good' | 'bad' | 'warn' }
interface AgendaMotive { icon: string; title: string; description: string; level: 'high' | 'medium' | 'low' }
...
export default function CheckerSection() {
  ...
  function shareResult() {
    if (navigator.share) {
      navigator.share({ title: 'SiteVerify Result', text: 'I just scanned a site with SiteVerify!', url: window.location.href })
    } else {
        ...
        <div className="mt-4 flex flex-wrap gap-2 items-center">
          ...
          {EXAMPLES.map(ex => (
            <button key={ex} onClick={() => setInput(ex)}
              className="text-xs font-mono text-gray-400 hover:text-green-400 border border-gray-700 px-2 py-1 rounded transition-colors">
              {ex}
            </button>
          ))}
        </div>
              ...
              {result.scores.map((s, i) => (
                <div key={i} className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
                  ...
                  <div className="h-1.5 bg-gray-100 rounded-full mt-2 overflow-hidden">
                    <div className={`h-full rounded-full ${scoreBg(s.value)}`} style={{ width: `${s.value}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
